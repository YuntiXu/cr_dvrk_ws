...
/* source line: 331 */
class CISST_EXPORT mtsComponentConstructorArg: public mtsGenericObject
{
 /* default constructors and destructors. */
 public:
    mtsComponentConstructorArg(void);
    mtsComponentConstructorArg(const mtsComponentConstructorArg & other);
    mtsComponentConstructorArg & operator = (const mtsComponentConstructorArg & other);
    ~mtsComponentConstructorArg();
...
/* source line: 343 */
 ...
 public:
    /* ctor-all-member is set to: true */
    mtsComponentConstructorArg(const std::string & newName);
    /* default methods */
 public:
    ...
    /* default data methods */
 public:
    void Copy(const mtsComponentConstructorArg & source);
    void SerializeBinary(std::ostream & outputStream) const CISST_THROW(std::runtime_error);
    void DeSerializeBinary(std::istream & inputStream, const cmnDataFormat & localFormat, const cmnDataFormat & remoteFormat) CISST_THROW(std::runtime_error);
    void SerializeText(std::ostream & outputStream, const char delimiter = ',') const CISST_THROW(std::runtime_error);
    std::string SerializeDescription(const char delimiter = ',', const std::string & userDescription = "") const;
...
typedef mtsGenericObjectProxy<mtsComponentConstructorArg> mtsComponentConstructorArgProxy;
CMN_DECLARE_SERVICES_INSTANTIATION(mtsComponentConstructorArgProxy);

/* default functions */
void CISST_EXPORT cmnSerializeRaw(std::ostream & outputStream, const mtsComponentConstructorArg & object);
void CISST_EXPORT cmnDeSerializeRaw(std::istream & inputStream, mtsComponentConstructorArg & placeHolder);
/* data functions */
template <> class cmnData<mtsComponentConstructorArg > {
public: 
    enum {IS_SPECIALIZED = 1};
    typedef mtsComponentConstructorArg DataType;
    static void Copy(DataType & data, const DataType & source) {
        data.Copy(source);
    }
    static std::string SerializeDescription(const DataType & data, const char delimiter, const std::string & userDescription) {
...
inline std::ostream & operator << (std::ostream & outputStream, const mtsComponentConstructorArg & data) {
    outputStream << cmnData<mtsComponentConstructorArg >::HumanReadable(data);
    return outputStream;
}
#if CISST_HAS_JSON
template <> void CISST_EXPORT cmnDataJSON<mtsComponentConstructorArg >::SerializeText(const mtsComponentConstructorArg & data, Json::Value & jsonValue);
template <> void CISST_EXPORT cmnDataJSON<mtsComponentConstructorArg >::DeSerializeText(mtsComponentConstructorArg & data, const Json::Value & jsonValue) CISST_THROW(std::runtime_error);
#endif // CISST_HAS_JSON
