...
// cisst version: 1.3.1
// source file: /home/ytxu/cr_dvrk_ws/src/cisst-saw/cisst/cisstMultiTask/code/../mtsExecutionResult.cdg
...
class CISST_EXPORT mtsExecutionResult
{
 ...
 public:
    ...
    mtsExecutionResult & operator = (const mtsExecutionResult & other);
    ~mtsExecutionResult();
...
static std::string EnumToString(const Enum & value) CISST_THROW(std::runtime_error);
static Enum EnumFromString(const std::string & value) CISST_THROW(std::runtime_error);
...
template <> void CISST_EXPORT cmnDataJSON<mtsExecutionResult >::DeSerializeText(mtsExecutionResult & data, const Json::Value & jsonValue) CISST_THROW(std::runtime_error);
#endif // CISST_HAS_JSON
